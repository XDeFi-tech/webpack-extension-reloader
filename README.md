...
```bash
npm install -D @xdefi-tech/webpack-extension-reloader
```

yarn

```bash
yarn add @xdefi-tech/webpack-extension-reloader --dev
```
...
Add `@xdefi-tech/webpack-extension-reloader` to the plugins section of your webpack configuration file. Note that this plugin don't outputs the manifest (at most read it to gather information).
For outputing not only the `manifest.json` but other static files too, use `CopyWebpackPlugin`.

```js
const ExtensionReloader = require("@xdefi-tech/webpack-extension-reloader");
